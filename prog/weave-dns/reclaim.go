...
func populateDNS2(ns *nameserver.Nameserver, dockerCli *weavedocker.Client, ourName mesh.PeerName, domainName string) (err error) {
	netBridge, err := dockerCli.GetNetwork("bridge")
	if netBridge != nil {

		//list containers in network

		netinfo, _ := dockerCli.NetworkInfo(netBridge.ID)

		for cid, container := range netinfo.Containers {
			fqdn := fmt.Sprintf("%v.%v", container.Name, domainName)
			ip, _, _ := net.ParseCIDR(container.IPv4Address)
			ns.AddEntryFQDN2(fqdn, cid, ourName, address.FromIP4(ip), true)
		}
	}
	return nil
}
